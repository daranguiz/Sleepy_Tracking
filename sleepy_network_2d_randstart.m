...
global sensx;
rng(sdx);
sensx = rand(1,NUM_SENSORS);
global statesx;
statesx = floor(((1:NUM_STATES) - 1)/sqrt(NUM_STATES))/(sqrt(NUM_STATES)-1);
global sensy;
rng(sdy);
sensy = rand(1,NUM_SENSORS);
global statesy;
statesy = mod((1:NUM_STATES) - 1,sqrt(NUM_STATES))/(sqrt(NUM_STATES)-1);

% Start the target in the first position.
b_k = floor(NUM_STATES/2)+floor(sqrt(NUM_STATES/2));

%% Network type independant initialization
global AWAKE_COST;
AWAKE_COST = 0.03;
global T;
PAUSE_TIME = 0.3;   % 0.3 is pretty good for visual inspection.
...
while (1)
    ...
    avg_sleep_arr(time) = sum(r(r~=Inf))/NUM_SENSORS;
    num_on_arr(time) = sum(r==0);
    
    % Compute a new posterior distribution
    prior = p_k*P;

    % Indices of sensors that gave observations
    indices = find(r==0);
...
detection_rate = (time-1-tcost)/(time-1)
Avg_Energy_Use = sum(num_on_arr)/(time-1)
